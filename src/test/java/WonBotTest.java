...
//        Sockets.waitForSocket("localhost", port, Constants.COMPONENT_STARTUP_WAIT);
    ...
    public void botsConnectToRepuationServer() throws Exception {
        ...
        try {
            Sockets.waitForSocket("localhost", alicePort, Constants.COMPONENT_STARTUP_WAIT);
            Sockets.waitForSocket("localhost", bobPort, Constants.COMPONENT_STARTUP_WAIT);
            // TODO wait for reputation server

        } catch (SocketTimeoutException e) {
            err.addError(new AssertionError("Expected a TCP server socket on port " + alicePort, e));
            err.addError(new AssertionError("Expected a TCP server socket on port " + bobPort, e));
        }
